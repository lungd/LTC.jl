...
function train_sine_node_s(n, solver=VCABM(), sensealg=InterpolatingAdjoint(autojacvec=ReverseDiffVJP(true)))
  ...
  # opt = GalacticOptim.Flux.Optimiser(ClipValue(0.5), ADAM(0.1), ExpDecay(0.1, 0.8, 40, 1e-5))
  opt = GalacticOptim.Flux.Optimiser(ClipValue(1.0), ADAM(0.02, (0.85, 0.8)))
  # opt = Optim.LBFGS()
  # opt = BBO()
...
@time model = train_sine_node_s(10000, Tsit5())
