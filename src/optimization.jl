load_model(chain::Flux.Chain, T=Float32) = (LTC.destructure(chain)..., get_bounds(chain,T)...)
load_model(chain::FastChain, T=Float32) = (initial_params(chain), chain, get_bounds(chain,T)...)

function optimize(chain, loss, cb, opt, AD, train_dl, epochs=1; normalize=false, T=Float32)
