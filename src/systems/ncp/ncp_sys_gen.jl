...
function SigmoidSynapse(T::DataType=Float32; name, E=rand_uniform(T, -0.8, 0.8))
...
function LeakChannel(T::DataType=Float32; name)
...
function Neuron(T::DataType=Float32; name)
  @variables begin
...
function Net(wiring::Wiring{T}; name) where T <: AbstractFloat
  ...
  for i in 1:wiring.n_sensory
    push!(neurons, Neuron(T; name=Symbol("n$(n)_SensoryNeuron")))
    n += 1
  end
  for i in 1:wiring.n_inter
    push!(neurons, Neuron(T; name=Symbol("n$(n)_InterNeuron")))
    n += 1
  end
  for i in 1:wiring.n_command
    push!(neurons, Neuron(T; name=Symbol("n$(n)_CommandNeuron")))
    n += 1
  end
  for i in 1:wiring.n_motor
    push!(neurons, Neuron(T; name=Symbol("n$(n)_MotorNeuron")))
    n += 1
  end
  ...
  for dst in 1:length(neurons)
    ...
    for src in 1:length(wiring.sens_mask[:,dst])
      ...
      syn = SigmoidSynapse(T; name=Symbol("s$(n)_x$(src)-->n$(dst)_SigmoidSynapse"),E=wiring.sens_pol[src,dst])
      n += 1
    ...
    for src in 1:length(wiring.syn_mask[:,dst])
      ...
      syn = SigmoidSynapse(T; name=Symbol("s$(n)_n$(src)-->n$(dst)_SigmoidSynapse"),E=wiring.syn_pol[src,dst])
      n += 1
